<template>
  <view class="phone-login-container">
    ...
    <view class="content">
        ...
        <view @click="getVcode" class="get_vcode" :style="{color: isCountDown ? '#a8a8a8' : 'black'}">{{tipName}}</view>
  ...
  methods: {
    async getVcode(){
      ...
      if(result.data.status === 0){
        uni.showToast({
          title: result.data.vcode+'', // 这里必须是string类型,uniapp文档中已写
          icon: 'none',
          duration: 1000
        })
      }
